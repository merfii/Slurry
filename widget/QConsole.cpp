	...
	promptLength(0), promptParagraph(0)
{
	...
	document()->setMaximumBlockCount(20);
...
void QConsole::displayMessage(const QString msg)
{
	setUndoRedoEnabled(false);
	...
	*/
	...
	if ((e->modifiers() & Qt::ControlModifier) && (e->key() == Qt::Key_C))
	{
		...
		default:
			...
			{ //no selection
				//when typing normal characters, make sure the cursor is positionned in the edition zone
				/*
				if (!isInEditionZone())
				{
					moveCursor(QTextCursor::End, QTextCursor::MoveAnchor);
