...
RobotState::RobotState(std::condition_variable* msg_cond):RobotState() {
...
void RobotState::unpackRobotMode(uint8_t * buf, unsigned int offset) {
	...
	if (RobotState::getVersion() > 2.) {
		...
		offset += sizeof(temp);
		robot_mode_.targetSpeedFraction = ntohd(temp);
	}
	memcpy(&temp, &buf[offset], sizeof(temp));
	offset += sizeof(temp);
	robot_mode_.speedScaling = ntohd(temp);
}
...
void RobotState::unpackRobotStateMasterboard(uint8_t * buf,
	...
	memcpy(&temp, &buf[offset], sizeof(temp));
	offset += sizeof(temp);
	mb_data_.analogInput0 = ntohd(temp);
	memcpy(&temp, &buf[offset], sizeof(temp));
	offset += sizeof(temp);
	mb_data_.analogInput1 = ntohd(temp);
	...
	memcpy(&temp, &buf[offset], sizeof(temp));
	offset += sizeof(temp);
	mb_data_.analogOutput0 = ntohd(temp);
	memcpy(&temp, &buf[offset], sizeof(temp));
	offset += sizeof(temp);
	mb_data_.analogOutput1 = ntohd(temp);
